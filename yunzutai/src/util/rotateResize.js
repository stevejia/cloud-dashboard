...
class RotateResize {
  constructor(editorIns) {
    ...
    // this._init(vm, elementCls);
  }

  init(vm, elementCls, symbolVm) {
    this._init(vm, elementCls, symbolVm);
  }
  _init(vm, elementCls, symbolVm) {
    this.symbolVm = symbolVm;
    this.mouseMoveBindThis = this.mouseMove.bind(this);
    this.mouseUpBindThis = this.mouseUp.bind(this);
    this.compData = vm;
    elementCls = elementCls || ".symbolDiv";
  ...
  resize(event, compData, index, scale) {
    _isResizing = true;
    this.scale = scale;
    let responsiveResize = event.shiftKey;
    ...
    let { left: x, top: y, width, height, transform: deg } = this.compData;
    let rect = {
      ox,
      oy,
      x,
    ...
    document.addEventListener("mousemove", this.mouseMoveBindThis, false);
    document.addEventListener("mouseup", this.mouseUpBindThis, false);
  }
  ...
  mouseUp(event) {
    ...
    event.stopPropagation();
    event.preventDefault();
    document.removeEventListener("mousemove", this.mouseMoveBindThis, false);
    document.removeEventListener("mouseup", this.mouseUpBindThis, false);
    //缩放结束后 记录一次历史
   ...
   */
  getSlope(linePoint1, linePoint2) {
   ...
   */
  getProjectivePoint(pOut, lineParam) {
    let { k, b, x } = lineParam;
    let { x: x0, y: y0 } = pOut;
   ...
   */
  _calcRotatedCordinate(current, center, deg) {
