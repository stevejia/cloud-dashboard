<template>
  <div class="symbolComp" :style="styles" @click="showDialog">
    <div class="title">
      <span :style="fontStyle">{{ baisicConfig["标题"].value }}</span>
      <span :style="fontStyle">当前温度：{{ highConfig.curTemp }}℃</span>
    </div>
    <ul class="btn-row" :class="theme">
      ...
      <li :style="style2">
        <span class="temperature">{{ highConfig.setTemp }}</span>
        <span class="text" :style="fontStyle">
          <span class="text-inner">设置温度</span>
        </span>
      </li>
      <li :style="style2">
        <span
      ...
      <li :style="style2">
        <i class="iconfont iconoff" :class="{ active: highConfig.state }"></i>
        <span class="text" :style="fontStyle">
          <span class="text-inner">{{ highConfig.state ? "开" : "关" }}</span>
        </span>
      </li>
    </ul>
    <el-dialog
...
import translateConfig from "./config/airConditioner3.js";
...
export default {
  ...
  methods: {
    ...
    initStyle(bool, type) {
      this.startInitStyle();
      this.styles = {
        fontSize: this.baisicConfig["尺寸"].value[1] * 0.77 + "px",
    ...
    resetComp() {
      ...
      this.styles.fontSize = height * 0.77 + "px";
