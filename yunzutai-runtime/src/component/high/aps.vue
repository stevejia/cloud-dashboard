<template>
  <div
    class="symbolComp"
    :style="styles"
    @contextmenu.prevent.stop="$drag.showCompContextMenu('aps', $parent)"
  >
      ...
      <template v-slot:oper="scope">
        <div>
          ...
          <template v-else-if="scope.row.state < 0">
            <span style="color:#e53f3f" @click="toChange(scope.row, 2)"
...
import translateConfig from "./swichConfig/aps.js";
export default {
  components: {
    TableComp
  },
  ...
  destroyed() {
    ...
    let bool = false;
    let apsList = this.$store.state.pointGroup.apsList.filter(item => {
      if (item.id !== "aps_" + compId) {
        return true;
      } else {
        bool = true;
      }
  ...
  methods: {
    ...
    init() {
      if (this.$store.state.runtime) {
        let compId = this.$parent.id;
        let target = this.$store.state.pointGroup.apsList.find(item => {
          return item.id === "aps_" + compId;
        });
        if (target) {
          this.$drag.dealAps(this, target);
        }
      } else {
        this.requestData();
      }
    ...
    toChange(row, state) {
      if (this.$store.state.runtime) {
        if (state === 2) {
          this.$drag.operComp = this.$parent;
          this.$drag.$vm.$refs.dialogAps.dialogVisible = true;
        } else {
          this.$drag.apsBulkCtrl(this.$parent.id, [
            {
              ctrlState: state,
              taskId: row.taskId
            }
