...
import translateConfig from "./config/linechart";
  ...
  methods: {
    ...
    pointIdChange(val) {
      ...
      for (let i = 0; i < list.length; i++) {
        let newData = this.sereisDatas[i].slice(-Number(num));
        let obj = {
          name: list[i].name,
          type: list[i].type,
      ...
      // moment(this.beginTime).format("YYYY-MM-DD HH:mm:ss")
